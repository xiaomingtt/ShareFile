...
def start_flask_server(p, fol, cip):
    ...
    def index():
        ...
        html8 = '''<!DOCTYPE html><html><head><meta charset="utf-8"><title>五子棋</title><style type="text/css">canvas{display:block;margin:50px auto;box-shadow:-2px -2px 2px #efefef,5px 5px 5px #b9b9b9;cursor:pointer}.btn-wrap{display:flex;flex-direction:row;justify-content:center}.btn-wrap div{margin:0 10px}div>span{display:inline-block;padding:10px 20px;color:#fff;background-color:#1296db;border-radius:5px;cursor:pointer}div.unable span{background:#d6d6d4;color:#adacaa}#result-wrap{text-align:center}</style></head><body><h3 id="result-wrap">--五子棋--</h3><canvas id="chess" width="450px" height="450px"></canvas><div class="btn-wrap"><div id="restart" class="restart"><span>重新开始</span></div><div id="goback" class="goback unable"><span>悔棋</span></div><div id="return" class="return unable"><span>撤销悔棋</span></div></div><script type="text/javascript" charset="utf-8">for(var over=!1,me=!0,_nowi=0,_nowj=0,_compi=0,_compj=0,_myWin=[],_compWin=[],backAble=!1,returnAble=!1,resultTxt=document.getElementById("result-wrap"),chressBord=[],i=0;i<15;i++){chressBord[i]=[];for(var j=0;j<15;j++)chressBord[i][j]=0}var myWin=[],computerWin=[],wins=[];for(i=0;i<15;i++){wins[i]=[];for(j=0;j<15;j++)wins[i][j]=[]}var count=0;for(i=0;i<15;i++)for(j=0;j<11;j++){for(var k=0;k<5;k++)wins[i][j+k][count]=!0;count++}for(i=0;i<15;i++)for(j=0;j<11;j++){for(k=0;k<5;k++)wins[j+k][i][count]=!0;count++}for(i=0;i<11;i++)for(j=0;j<11;j++){for(k=0;k<5;k++)wins[i+k][j+k][count]=!0;count++}for(i=0;i<11;i++)for(j=14;3<j;j--){for(k=0;k<5;k++)wins[i+k][j-k][count]=!0;count++}for(i=0;i<count;i++)myWin[i]=0,_myWin[i]=0,computerWin[i]=0,_compWin[i]=0;var chess=document.getElementById("chess"),context=chess.getContext("2d");context.strokeStyle="#bfbfbf";var backbtn=document.getElementById("goback"),returnbtn=document.getElementById("return");window.onload=function(){drawChessBoard()},document.getElementById("restart").onclick=function(){window.location.reload()},chess.onclick=function(o){if(!over&&me){backbtn.className=backbtn.className.replace(new RegExp("(\\s|^)unable(\\s|$)")," ");var n=o.offsetX,e=o.offsetY,t=Math.floor(n/30),r=Math.floor(e/30);if(_nowj=r,0==chressBord[_nowi=t][r]){oneStep(t,r,me),chressBord[t][r]=1;for(var i=0;i<count;i++)wins[t][r][i]&&(myWin[i]++,_compWin[i]=computerWin[i],computerWin[i]=6,5==myWin[i]&&(resultTxt.innerHTML="恭喜，你赢了！",over=!0));over||(me=!me,computerAI())}}},backbtn.onclick=function(o){if(backAble){me=!(over=!1),returnbtn.className=returnbtn.className.replace(new RegExp("(\\s|^)unable(\\s|$)")," "),chressBord[_nowi][_nowj]=0,minusStep(_nowi,_nowj);for(var n=0;n<count;n++)wins[_nowi][_nowj][n]&&(myWin[n]--,computerWin[n]=_compWin[n]);chressBord[_compi][_compj]=0,minusStep(_compi,_compj);for(n=0;n<count;n++)wins[_compi][_compj][n]&&(computerWin[n]--,myWin[n]=_myWin[i]);resultTxt.innerHTML="--益智五子棋--",backAble=!(returnAble=!0)}},returnbtn.onclick=function(o){if(returnAble){chressBord[_nowi][_nowj]=1,oneStep(_nowi,_nowj,me);for(var n=0;n<count;n++)wins[_nowi][_nowj][n]&&(myWin[n]++,_compWin[n]=computerWin[n],computerWin[n]=6),5==myWin[n]&&(resultTxt.innerHTML="恭喜，你赢了！",over=!0);chressBord[_compi][_compj]=2,oneStep(_compi,_compj,!1);for(n=0;n<count;n++)wins[_compi][_compj][n]&&(computerWin[n]++,_myWin[n]=myWin[n],myWin[n]=6),5==computerWin[n]&&(resultTxt.innerHTML="你输了，继续加油哦！",over=!0);returnbtn.className+=" unable",backAble=!(returnAble=!1)}};var computerAI=function(){for(var o=[],n=[],e=0,t=0,r=0,i=0;i<15;i++){o[i]=[],n[i]=[];for(var c=0;c<15;c++)o[i][c]=0,n[i][c]=0}for(i=0;i<15;i++)for(c=0;c<15;c++)if(0==chressBord[i][c]){for(var m=0;m<count;m++)wins[i][c][m]&&(1==myWin[m]?o[i][c]+=200:2==myWin[m]?o[i][c]+=400:3==myWin[m]?o[i][c]+=2e3:4==myWin[m]&&(o[i][c]+=1e4),1==computerWin[m]?n[i][c]+=220:2==computerWin[m]?n[i][c]+=420:3==computerWin[m]?n[i][c]+=2100:4==computerWin[m]&&(n[i][c]+=2e4));o[i][c]>e?(e=o[i][c],t=i,r=c):o[i][c]==e&&n[i][c]>n[t][r]&&(t=i,r=c),n[i][c]>e?(e=n[i][c],t=i,r=c):n[i][c]==e&&o[i][c]>o[t][r]&&(t=i,r=c)}oneStep(_compi=t,_compj=r,!1),chressBord[t][r]=2;for(m=0;m<count;m++)wins[t][r][m]&&(computerWin[m]++,_myWin[m]=myWin[m],myWin[m]=6,5==computerWin[m]&&(resultTxt.innerHTML="你输了，继续加油哦！",over=!0));over||(me=!me),returnAble=!(backAble=!0),new RegExp("unable").test(" "+returnbtn.className+" ")||(returnbtn.className+=" unable")},drawChessBoard=function(){for(var o=0;o<15;o++)context.moveTo(15+30*o,15),context.lineTo(15+30*o,435),context.stroke(),context.moveTo(15,15+30*o),context.lineTo(435,15+30*o),context.stroke()},oneStep=function(o,n,e){context.beginPath(),context.arc(15+30*o,15+30*n,13,0,2*Math.PI),context.closePath();var t=context.createRadialGradient(15+30*o+2,15+30*n-2,13,15+30*o+2,15+30*n-2,0);e?(t.addColorStop(0,"#0a0a0a"),t.addColorStop(1,"#636766")):(t.addColorStop(0,"#d1d1d1"),t.addColorStop(1,"#f9f9f9")),context.fillStyle=t,context.fill()},minusStep=function(o,n){context.clearRect(30*o,30*n,30,30),context.beginPath(),context.moveTo(15+30*o,30*n),context.lineTo(15+30*o,30*n+30),context.moveTo(30*o,30*n+15),context.lineTo(30*(o+1),30*n+15),context.stroke()}</script></body></html>'''
        html9 = '''<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd"><html><head><meta charset="UTF-8"><title>俄罗斯方块</title><style type="text/css">*{box-sizing:border-box;margin:0;padding:0}body{width:530px;margin:20px auto;background-color:#006030;font-family:'微软雅黑',sans-serif;color:#fff}table#gameBoard{float:left;margin-right:20px;border:1px solid #000;border-collapse:collapse;background-color:#fff;border-radius:5px;overflow:hidden}table#gameBoard td{width:30px;height:30px;border:1px dotted #0cc;text-align:center}.font{font-family:'微软雅黑',sans-serif;font-size:18px;text-align:center}div#gameControl{width:160px;float:right;padding:20px;background:linear-gradient(145deg,#00a650,#007b47);border:2px solid #00573c;border-radius:10px;box-shadow:0 4px 8px rgba(0,0,0,.2);text-align:center}div#gameControl span#score{font-size:24px;font-weight:700;color:#fff;background:rgba(255,255,255,.2);padding:10px 15px;border-radius:5px;display:block;margin-bottom:20px}div#gameControl input[type=button]{width:100px;height:35px;background-color:#0cc;color:#fff;border:none;border-radius:5px;font-size:16px;cursor:pointer;transition:background-color .3s,transform .2s}div#gameControl input[type=button]:hover{background-color:#099;transform:scale(1.05)}div#gameControl input[type=button]:active{transform:scale(.98)}</style><script type="text/javascript">var T=TETRIS={aBoardGrids:[],aShapes:[[52224],[34952,3840],[35904,27648],[19584,50688],[17600,36352,51328,57856],[35008,59392,50240,11776],[19968,35968,58368,19520]],init:function(){this.oDomBoard=document.getElementById("gameBoard"),this.oDomScore=document.getElementById("score"),this.aBoardGrids=new Array(18);for(var e=0;e<18;e++){this.aBoardGrids[e]=new Array(10);for(var r=this.oDomBoard.insertRow(-1),t=0;t<10;t++)this.aBoardGrids[e][t]=0,r.insertCell(t)}document.onkeydown=function(e){var r;switch((e=e||window.event).which||e.keyCode){case 37:T.oBlock.move("left");break;case 38:T.oBlock.rotate((r=T.aShapes[T.iShapeIdx][++T.index%T.aShapes[T.iShapeIdx].length].toString(2),r=new Array(17-r.length).join(0)+r,T.matrix=r.match(/\d{4}/g),T.matrix));break;case 39:T.oBlock.move("right");break;case 40:T.oBlock.move("down")}}},next:function(){this.iShapeIdx=parseInt(Math.random()*this.aShapes.length),this.index=0;var e=this.aShapes[this.iShapeIdx][this.index].toString(2);e=new Array(17-e.length).join(0)+e,this.matrix=e.match(/\d{4}/g),this.oBlock=new TETRIS.Block(this.matrix),clearInterval(T.timer),T.timer=setInterval(function(){T.oBlock.move("down")},1e3),T.oBlock.checkBlock()||(alert("游戏结束"),clearInterval(T.timer))},updateBoard:function(){for(var e=0;e<4;e++)this.aBoardGrids[T.oBlock.shape[e].y][T.oBlock.shape[e].x]=1},eraseLines:function(){for(var e=0,r=17;0<=r;r--){for(var t=0,o=0;o<10;o++)1==this.aBoardGrids[r][o]&&t++;if(10==t){e++;for(var a=0;a<o;a++){for(var i=r;0<i;i--)this.aBoardGrids[i][a]=this.aBoardGrids[i-1][a],T.oDomBoard.rows[i].cells[a].style.background=T.oDomBoard.rows[i-1].cells[a].style.background;this.aBoardGrids[0][a]=0}r++}}return e},setScore:function(e){var r=parseInt(this.oDomScore.innerHTML);1==e?r+=100:2==e?r+=300:3==e?r+=500:4==e&&(r+=1e3),this.oDomScore.innerHTML=r}};TETRIS.Block=function(a){this.shape=function(){for(var e=[],r=0;r<a.length;r++)for(var t=a[r],o=0;o<t.length;o++)"1"==t.charAt(o)&&e.push({x:o+3,y:r});return e}(),this.draw()},TETRIS.Block.prototype.move=function(e){if(this.checkBlock(this.shape,e)){this.draw("clear");for(var r=0;r<4;r++)switch(e){case"left":this.shape[r].x--;break;case"right":this.shape[r].x++;break;case"down":this.shape[r].y++}this.draw()}else if("down"==e){this.draw(),T.updateBoard();var t=T.eraseLines();0<t&&T.setScore(t),T.next()}},TETRIS.Block.prototype.rotate=function(c){this.shape=function(e){for(var r=[],t=[],o=0;o<4;o++)r.push(e.shape[o].x),t.push(e.shape[o].y);var a=r.getMin(),i=t.getMin(),n=[];for(o=0;o<c.length;o++)for(var s=c[o],h=0;h<s.length;h++)"1"==s.charAt(h)&&n.push({x:h+a,y:o+i});return e.checkBlock(n)?(e.draw("clear"),n):e.shape}(this),this.draw()},TETRIS.Block.prototype.draw=function(e){for(var r=0;r<this.shape.length;r++){var t=this.shape[r];T.oDomBoard.rows[t.y].cells[t.x].style.background=null==e?"#09F":""}},TETRIS.Block.prototype.checkBlock=function(e,r){e=e||this.shape;for(var t=0;t<4;t++){if("left"==r){if(0==e[t].x||1==T.aBoardGrids[e[t].y][e[t].x-1])return!1}else if("right"==r){if(9==e[t].x||1==T.aBoardGrids[e[t].y][e[t].x+1])return!1}else if("down"==r&&(17==e[t].y||1==T.aBoardGrids[e[t].y+1][e[t].x]))return!1;if(e[t].x<0||9<e[t].x||e[t].y<0||17<e[t].y)return!1;if(1==T.aBoardGrids[e[t].y][e[t].x])return!1}return!0},Array.prototype.getMin=function(){for(var e=this[0],r=0;r<this.length;r++)this[r]<e&&(e=this[r]);return e},window.onload=function(){T.init(),document.getElementById("btnPlay").onclick=function(){"开始"==this.value?(T.next(),this.value="结束"):(this.value="开始",alert("游戏结束"),clearInterval(T.timer))},document.getElementById("btnPause").onclick=function(){"暂停"==this.value?(clearInterval(T.timer),this.value="继续"):(T.timer=setInterval(function(){T.oBlock.move("down")},1e3),this.value="暂停")}}</script></head><body><table id="gameBoard"></table><div id="gameControl">分数： <span id="score">0</span><br><input type="button" id="btnPlay" value="开始"> <input type="button" id="btnPause" value="暂停"></div></body></html>'''
        return random.choice([html1, html2, html3, html4, html5, html6, html7, html8, html9])
        
    @app.errorhandler(404)
    def page_not_found(e):
        return '''<!DOCTYPE html><html><head><meta charset="utf-8"><title>页面未找到</title><style type="text/css">body,html{margin:0;padding:0;height:100%;min-height:450px;font-family:Dosis,sans-serif;font-size:32px;font-weight:500;color:#5d7399}.content{height:100%;position:relative;z-index:1;background-color:#d2e1ec;background-image:linear-gradient(to bottom,#bbcfe1 0,#e8f2f6 80%);overflow:hidden}.snow{position:absolute;top:0;left:0;pointer-events:none;z-index:20}.main-text{padding:20vh 20px 0 20px;text-align:center;line-height:2em;font-size:5vh}.home-link{font-size:.6em;font-weight:400;color:inherit;text-decoration:none;opacity:.6;border-bottom:1px dashed rgba(93,115,153,.5)}.home-link:hover{opacity:1}.ground{height:160px;width:100%;position:absolute;bottom:0;left:0;background:#f6f9fa;box-shadow:0 0 10px 10px#f6f9fa}.ground:after,.ground:before{content:"";display:block;width:250px;height:250px;position:absolute;top:-62.5px;z-index:-1;background:0 0;transform:scaleX(.2) rotate(45deg)}.ground:after{left:50%;margin-left:-166.6666666667px;box-shadow:-340px 260px 15px#bac4d5,-625px 575px 15px#91a1bc,-855px 945px 15px#7e90b0,-1165px 1235px 15px#b0bccf,-1470px 1530px 15px#94a3be,-1750px 1850px 15px#91a1bc,-2145px 2055px 15px#b0bccf,-2400px 2400px 15px#7e90b0,-2665px 2735px 15px#a7b4c9,-2965px 3035px 15px#8496b4,-3260px 3340px 15px#94a3be,-3580px 3620px 15px#97a6c0,-3885px 3915px 15px#9aa9c2,-4160px 4240px 15px#8193b2,-4470px 4530px 15px#8e9eba,-4845px 4755px 15px#7e90b0}.ground:before{right:50%;margin-right:-166.6666666667px;box-shadow:260px-340px 15px#b0bccf,630px-570px 15px#a1aec6,925px-875px 15px#94a3be,1170px-1230px 15px#a7b4c9,1535px-1465px 15px#a7b4c9,1845px-1755px 15px#8a9bb8,2150px-2050px 15px#b7c1d3,2445px-2355px 15px#8798b6,2735px-2665px 15px#bac4d5,3015px-2985px 15px#94a3be,3270px-3330px 15px#b7c1d3,3620px-3580px 15px#8193b2,3860px-3940px 15px#9dabc4,4215px-4185px 15px#8798b6,4485px-4515px 15px#8e9eba,4810px-4790px 15px#bac4d5}.mound{margin-top:-100px;font-weight:800;font-size:180px;text-align:center;color:#dd4040;pointer-events:none}.mound:before{content:"";display:block;width:600px;height:200px;position:absolute;left:50%;margin-left:-300px;top:50px;z-index:1;border-radius:100%;background-color:#e8f2f6;background-image:linear-gradient(to bottom,#dee8f1,#f6f9fa 60px)}.mound:after{content:"";display:block;width:28px;height:6px;position:absolute;left:50%;margin-left:-150px;top:68px;z-index:2;background:#dd4040;border-radius:100%;transform:rotate(-15deg);box-shadow:-56px 12px 0 1px#dd4040,-126px 6px 0 2px#dd4040,-196px 24px 0 3px#dd4040}.mound_text{transform:rotate(10deg)}.mound_spade{display:block;width:35px;height:30px;position:absolute;right:50%;top:42%;margin-right:-250px;z-index:0;transform:rotate(35deg);background:#dd4040}.mound_spade:after,.mound_spade:before{content:"";display:block;position:absolute}.mound_spade:before{width:40%;height:30px;bottom:98%;left:50%;margin-left:-20%;background:#dd4040}.mound_spade:after{width:100%;height:30px;top:-55px;left:0;box-sizing:border-box;border:10px solid#dd4040;border-radius:4px 4px 20px 20px}</style></head><body><div class="content"><canvas class="snow" id="snow"></canvas><div class="main-text"><h2>哎呀！<br>你访问的页面走丢啦！</h2></div><div class="ground"><div class="mound"><div class="mound_text">404</div><div class="mound_spade"></div></div></div></div><script type="text/javascript">!function(){function t(t){var n=t.getContext("2d"),e=0,i=0,o=[],d=function(){this.x=this.y=this.dx=this.dy=0,this.reset()};function a(){e=window.innerWidth,i=window.innerHeight,t.width=e,t.height=i,function(t){if(t!=o.length){o=[];for(var n=0;n<t;n++)o.push(new d)}}(e*i/1e4)}d.prototype.reset=function(){this.y=Math.random()*i,this.x=Math.random()*e,this.dx=1*Math.random()-.5,this.dy=.5*Math.random()+.5},a(),function t(){n.clearRect(0,0,e,i),n.fillStyle="#f6f9fa",o.forEach(function(t){t.y+=t.dy,t.x+=t.dx,t.y>i&&(t.y=0),t.x>e&&(t.reset(),t.y=0),n.beginPath(),n.arc(t.x,t.y,5,0,2*Math.PI,!1),n.fill()}),window.requestAnimationFrame(t)}(),window.addEventListener("resize",a)}var n;n=function(){t(document.getElementById("snow"))},"loading"!=document.readyState?n():document.addEventListener("DOMContentLoaded",n)}()</script></body></html>'''
